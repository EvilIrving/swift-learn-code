...
struct 定存: HomeworkProtocol {
    ...
    var 期滿領回: Int {
        let money = (1 ... 年數).reduce(Double(金額)) { money, _ in
            money * (1.0 + rate)
        }
        return Int(money)
    }
    ...
    var 報酬率: Double {
        return Double(期滿領回 - 金額) / Double(金額)
    }

    var 描述: String {
        return "金额：\(金額)，年數：\(年數)，期滿領回：\(期滿領回)，報酬率：\(報酬率)"
    }

    var rate: Double {
        let base = Self.基礎利率對照表.last(where: { 金額 >= $0.最小金額 })!.利率
        let plus = Self.年數利率加成對照.last(where: { 年數 >= $0.最小年數 })!.利率
        return base + plus
    }
