...
Swift 代码有 100 字符单行限制。除了下面的说明之外，任何超过该限制的行都需要换行，详情见 [换行](#换行)。
...
* 左花括号（`{`）之前**不需要**换行，**除非**是为了满足 [换行](#换行) 规则。
...
* 右花括号（`}`）之前**需要**换行，除非可以省略成 [单行语句](#单行语句) 或是空白块。
...
如果要定一个“命名空间”来对互相关联的声明（例如常量或者帮助方法）进行分组，公认的做法是声明一个没有枚举项的 `enum`。该 `enum` 自然不存在实例，并且不需要额外的样板代码来避免被实例化。
...
如果你希望用和原本不同的含义重载已存在的运算符，参考 [定义新运算符](#定义新运算符) 指引来确定是否允许。也就是说，如果新的含义在问题领域是确定已久的，并且使用该运算符会比其他语法结构提高可读性，那么就是允许的。
