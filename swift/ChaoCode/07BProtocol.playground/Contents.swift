...
protocol 交通工具 /*: Equatable, CustomStringConvertible */ {
    var 最大乘客數: Int { get }
    var 牌號: String { get set }
    var 時速: Double { get }
...
struct 汽車: 交通工具, CustomStringConvertible {
    var 最大乘客數: Int = 4
    var 牌號: String
    let 時速: Double

    var description: String { "🚘 汽車牌號「\(牌號)」" }
}

struct 機車: 交通工具, CustomStringConvertible {
    let 最大乘客數: Int = 2
    var 牌號: String
    let 時速: Double

    var description: String { "🛵 機車牌號「\(牌號)」" }
}

struct 直升機: 交通工具, CustomStringConvertible {
    let 最大乘客數: Int = 4
    var 牌號: String
    let 時速: Double

    var description: String { "🚁 直升機牌號「\(牌號)」" }
}
...
struct 載客管理員 {
    ...
    mutating func 派車(人數: Int, 客人距離: Double) -> 交通工具? {
//        if 可用載客車.isEmpty {
//            print("😣 很抱歉，目前沒有車輛。請稍後再試。")
//            return nil
//        }
        let curIdx = 可用載客車.firstIndex(where: { $0.最大乘客數 >= 人數 })
        guard let curIdx = curIdx else {
            print("😣 很抱歉，目前沒有車輛。請稍後再試。")
            return nil
        }

        let vehicle = 可用載客車[curIdx]
        可用載客車.remove(at: curIdx)
        print("\(vehicle)正在前往您的地點，預計 \(vehicle.計算時間(目的地距離: 客人距離)) 分鐘後抵達。")
        return vehicle
    }
}
...
var carManager = 載客管理員(可用載客車:
    [汽車(最大乘客數: 4, 牌號: "TDG-1688", 時速: 80),
     機車(牌號: "LKK-0057", 時速: 60),
     機車(牌號: "AWJ-0020", 時速: 60),
     汽車(最大乘客數: 3, 牌號: "TDZ-2096", 時速: 80),
     汽車(最大乘客數: 5, 牌號: "TRT-4042", 時速: 70),
     機車(牌號: "LMI-0009", 時速: 80),
     直升機(牌號: "B-70331", 時速: 293),
     汽車(最大乘客數: 5, 牌號: "TJU-2244", 時速: 60),
     汽車(最大乘客數: 4, 牌號: "TTN-6433", 時速: 80),
     汽車(最大乘客數: 4, 牌號: "THK-9005", 時速: 85),
     機車(牌號: "PXP-2273", 時速: 55),
     機車(牌號: "VOJ-3635", 時速: 80),
     機車(牌號: "CDH-1960", 時速: 60),
     汽車(最大乘客數: 3, 牌號: "TFJ-6039", 時速: 65),
     汽車(最大乘客數: 3, 牌號: "TNK-0880", 時速: 85),
     機車(牌號: "UUE-2080", 時速: 50),
     機車(牌號: "VCE-8777", 時速: 75),
     機車(牌號: "BBD-3494", 時速: 60),
     汽車(最大乘客數: 6, 牌號: "TLZ-7005", 時速: 75),
     汽車(最大乘客數: 4, 牌號: "TWE-5773", 時速: 60),
     直升機(牌號: "B-70921", 時速: 301),
     機車(牌號: "HHC-2069", 時速: 60),
     機車(牌號: "ROW-4209", 時速: 65)])
